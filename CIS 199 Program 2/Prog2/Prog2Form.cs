...
namespace Prog2
{
    public partial class Prog2Form : Form
    {
        ...
        private void calcTaxBtn_Click(object sender, EventArgs e)
        {
            ...
            // Candidate 1 & 3
            const decimal C1_3_RATE1 = .10m;  // 1st tax rate (LOWEST)
            const decimal C1_3_RATE2 = .15m;  // 2nd tax rate
            const decimal C1_3_RATE3 = .25m;  // 3rd tax rate
            const decimal C1_3_RATE4 = .28m;  // 4th tax rate
            const decimal C1_3_RATE5 = .33m;  // 5th tax rate
            const decimal C1_3_RATE6 = .35m;  // 6th tax rate
            const decimal C1_3_RATE7 = .396m; // 7th tax rate (HIGHEST)
            ...
            //Candidate 1 and 3
            const int C1_3_THRESH1 = 9_525;   // 1st threshold (LOWEST)
            const int C1_3_THRESH2 = 38_700;  // 2nd threshold
            const int C1_3_THRESH3 = 82_500;  // 3rd threshold
            const int C1_3_THRESH4 = 157_500; // 4th threshold
            const int C1_3_THRESH5 = 200_000; // 5th threshold
            const int C1_3_THRESH6 = 500_000; // 6th threshold (HIGHEST)
            ...
            const decimal C2_EXTRA_RATE = .04m; // Candidate 2 Extra Tax Rate
            const int C2_EXTRA_THRESH = 29000;  // Candidate 2 Extra Tax Threshold
            const decimal C3_REDUCTION_RATE = .10m; // Candidate 3 Tax Reduction Percentage
            ...
            if (int.TryParse(incomeTxt.Text, out income) && income >= 0)
            {
                ...
                else if (candidate1RdoBtn.Checked || candidate3RdoBtn.Checked)// Candidate 1 or 3?
                {
                    rate1 = C1_3_RATE1;
                    rate2 = C1_3_RATE2;
                    rate3 = C1_3_RATE3;
                    ...
                    // Special Circumstances for Candidate 3
                    if (candidate3RdoBtn.Checked) // Additional for Candidate 3
                    {
